...
public class StoresAssertionTest {
    ...
    @Test
    public void test001() {
        response.body("total", Matchers.equalTo(1561));
    }
    ...
    @Test
    public void test003() {
        response.body("data.name", Matchers.hasItem("Inver Grove Heights"));
    ...
    @Test
    public void test005(){
        ValidatableResponse body = response.body("data[2].services[1].storeservices.storeId", Matchers.equalTo(7));

    }
    //Check hash map values ‘createdAt’ inside storeservices map where store name = Roseville
    @Test
    public void test006() {
        response.body("data.findAll{it.name=='Roseville'}.services.storeservices", Matchers.hasItem(Matchers.hasItem(hasKey("createdAt"))));
    }
